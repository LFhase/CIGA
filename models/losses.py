...
def get_contrast_loss(causal_rep, labels, norm=None, contrast_t=1.0, sampling='mul', y_pred=None, ratio=1.0):
    ...
    if sampling.lower() in ['mul', 'var']:
        ...
        # correct & contrast with only hard postive https://arxiv.org/abs/2203.01517
        device = causal_rep.device
        mask = torch.eq(labels.unsqueeze(1), labels.unsqueeze(1).T).float().to(device)
        # compute logits
        anchor_dot_contrast = torch.div(torch.matmul(causal_rep, causal_rep.T), contrast_t)
        ...
        pos_mask = y_pred != labels
        neg_mask = y_pred == labels
        # print(pos_mask.size(), neg_mask, y_pred.size(), labels.size())
        # hard negative: diff label && correct pred
        ...
        neg_exp_logits = torch.exp(logits) * neg_mask
        pos_exp_logits = torch.exp(logits) * pos_mask
        # print(torch.isnan(neg_exp_logits).sum(), (pos_exp_logits.sum(1, keepdim=True) + \
        # neg_exp_logits.sum(1, keepdim=True)<0).sum(),
        # torch.isnan(pos_exp_logits.sum(1, keepdim=True) + \
        # neg_exp_logits.sum(1, keepdim=True)).sum())
        # print(torch.isnan(pos_exp_logits).sum())
        # print(pos_mask.sum(), neg_mask.sum())

        log_prob = logits - \
                    torch.log(pos_exp_logits.sum(1, keepdim=True) + \
                            neg_exp_logits.sum(1, keepdim=True)+1e-12)
        # print(pos_exp_logits.sum(1, keepdim=True))
        # print(neg_exp_logits.sum(1, keepdim=True))
        # print(torch.log(pos_exp_logits.sum(1, keepdim=True) + \
        #                     neg_exp_logits.sum(1, keepdim=True)))
        # print(logits.size())
        # compute mean of log-likelihood over positive
        is_valid = pos_mask.sum(1) != 0
        # print(is_valid.sum(), (neg_mask.sum(1)[is_valid] != 0).sum())
        # mean_log_prob_pos = log_prob
        # print((pos_mask * log_prob).sum(1)[is_valid])
        mean_log_prob_pos = (pos_mask * log_prob).sum(1)[is_valid] / pos_mask.sum(1)[is_valid]
        # some classes may not be sampled by more than 2
        # mean_log_prob_pos[torch.isnan(mean_log_prob_pos)] = 0.0
        # print(mean_log_prob_pos)
        ...
        # print(is_valid.sum())
        # print(torch.isnan(mean_log_prob_pos).sum())
        # loss
        # contrast_loss = -(args.temperature / args.base_temperature) * mean_log_prob_pos
        # contrast_loss = contrast_loss.view(anchor_count, batch_size).mean()
        contrast_loss = -mean_log_prob_pos.mean()
        # print(contrast_loss)
    elif sampling.lower() == 'late':
        # contrasting top ceil(r*dim) vectors
        device = causal_rep.device
        mask = torch.eq(labels.unsqueeze(1), labels.unsqueeze(1).T).float().to(device)
        # compute logits
        anchor_dot_contrast = torch.div(torch.matmul(causal_rep, causal_rep.T), contrast_t)
        pos_dim = int(causal_rep.size(-1) * ratio)
        pos_causal_rep = causal_rep[:, :pos_dim]
        pos_anchor_dot_contrast = torch.div(torch.matmul(pos_causal_rep, pos_causal_rep.T), contrast_t)
        # for numerical stability
        pos_logits_max, _ = torch.max(pos_anchor_dot_contrast, dim=1, keepdim=True)
        pos_logits = pos_anchor_dot_contrast - pos_logits_max.detach()
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()
